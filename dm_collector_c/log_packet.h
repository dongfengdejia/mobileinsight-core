...
enum FmtType {
    ...
    PLACEHOLDER, // This field is created with a dummy value (no byte is
                 // consumed)
...
const Fmt LogPacketHeaderFmt[] = {{SKIP, NULL, 2},
                                  {UINT, "log_msg_len", 2},
                                  {UINT, "type_id", 2},
...
const Fmt WcdmaCellIdFmt[] = {
    ...
    {UINT, "UTRA registration area (overlapping URAs)",
     1},             // UTRA registration area (overlapping URAs)
...
// ------------------------------------------------------------
const Fmt WcdmaSignalingMessagesFmt[] = {{UINT, "Channel Type", 1},
...
const ValueName ValueNameWcdmaExtraSIBType[] = {
    {0, "RRC_MIB"},    {1, "RRC_SIB1"},   {2, "RRC_SIB2"},
    {3, "RRC_SIB3"},   {5, "RRC_SIB5"},   {7, "RRC_SIB7"},
    {11, "RRC_SIB11"}, {12, "RRC_SIB12"}, {19, "RRC_SIB19"},
};
...
// ------------------------------------------------------------
// UMTS NAS GMM State
const Fmt UmtsNasGmmStateFmt[] = {{UINT, "GMM State", 1},
                                  {UINT, "GMM Substate", 1},
...
const Fmt UmtsNasMmRegStateFmt[] = {{UINT, "Network operation mode", 1},
...
const Fmt UmtsNasMmStateFmt[] = {{UINT, "MM State", 1},
                                 {UINT, "MM Substate", 1},
...
const ValueName UmtsNasMmState_MmSubstate[] = {
    {0, "MM_NULL_SUBSTATE"},        {1, "MM_NO_IMSI"},
    {2, "MM_PLMN_SEARCH"},          {3, "MM_LIMITED_SERVICE"},
    {4, "MM_ATTEMPTING_TO_UPDATE"}, {5, "MM_LOCATION_UPDATE_NEEDED"},
    {6, "MM_NO_CELL_AVAILABLE"},    {7, "MM_PLMN_SEARCH_NORMAL_SERVICE"},
    {8, "MM_NORMAL_SERVICE"}};

const ValueName UmtsNasMmState_MmUpdateStatus[] = {
    {0, "UPDATED"}, {1, "NOT_UPDATED"}, {2, "ROAMING_NOT_ALLOWED"}};

// ------------------------------------------------------------
// UMTS NAS MM State
const Fmt UmtsNasOtaFmt[] = {
    {UINT, "Message Direction", 1}, {UINT, "Message Length", 4}
    // Followed by NAS messages
};

const ValueName UmtsNasOtaFmt_MessageDirection[] = {{0, "TO_UE"},
                                                    {1, "FROM_UE"}};
...
const ValueName LteRrcOtaPduType[] = {
    {0x02, "LTE-RRC_BCCH_DL_SCH"}, {0x04, "LTE-RRC_PCCH"},
    {0x05, "LTE-RRC_DL_CCCH"},     {0x06, "LTE-RRC_DL_DCCH"},
    {0x07, "LTE-RRC_UL_CCCH"},     {0x08, "LTE-RRC_UL_DCCH"},
};

const ValueName LteRrcOtaPduType_v15[] = {
    {0x02, "LTE-RRC_BCCH_DL_SCH"}, {0x05, "LTE-RRC_PCCH"},
    {0x06, "LTE-RRC_DL_CCCH"},     {0x07, "LTE-RRC_DL_DCCH"},
    {0x08, "LTE-RRC_UL_CCCH"},     {0x09, "LTE-RRC_UL_DCCH"},
};

// ------------------------------------------------------------
// LTE NAS Plain
const Fmt LteNasPlainFmt[] = {{UINT, "Pkt Version", 1}};
...
// ------------------------------------------------------------
// LTE NAS EMM State
const Fmt LteNasEmmStateFmt[] = {{UINT, "Version", 1}};

const Fmt LteNasEmmStateFmt_v2[] = {{UINT, "EMM State", 1},
                                    {UINT, "EMM Substate", 2},
                                    {PLMN_MK2, "PLMN", 3},
                                    {UINT, "GUTI Valid", 1},
                                    {UINT, "GUTI UE Id", 1},
...
const Fmt LteNasEsmStateFmt[] = {{UINT, "Version", 1}};
...
const ValueName LteNasEmmState_v2_EmmSubstate_Registered_Initiated[] = {
    {0, "EMM_WAITING_FOR_NW_RESPONSE"}, {1, "EMM_WAITING_FOR_ESM_RESPONSE"}};
...
// ------------------------------------------------------------
const Fmt LtePhyPdschDemapperConfigFmt[] = {{UINT, "Version", 1}};
...
const ValueName LtePhyPdschDemapperConfig_v23_Carrier_Index[] = {{0, "PCC"},
                                                                 {1, "SCC"}};
...
const Fmt LtePhySubpktFmt_v1_Scmr_v4[] = {
    ...
    {UINT, "RSRP Rx[0]",
     4}, // skip 10 bits, then 12 bits. (0.0625 * x - 180) dBm
    {UINT, "RSRP Rx[1]",
     4},               // skip 12 bits, then 12 bits (0.0625 * x - 180) dBm
    ...
    {PLACEHOLDER, "RSRQ",
     0}, // skip 20 bits, then 10 bits, (0.0625 * x - 30) dB
    {UINT, "RSSI Rx[0]",
     4}, // skip 10 bits, them 11 bits (0.0625 * x - 110) dBm
    {PLACEHOLDER, "RSSI Rx[1]",
     0},               // skip 21 bits, then 11 bits (0.0625 * x - 110) dBm
    {UINT, "RSSI", 4}, // 11 bits, (0.0625 * x - 110) dBm
    {SKIP, NULL, 20},
    {UINT, "FTL SNR Rx[0]", 4}, // 9 bits
    {PLACEHOLDER, "FTL SNR Rx[1]",
     0}, // skip 9 bits, then 9 bits (0.1 * x - 20) dB
    {SKIP, NULL, 12},
};
...
const Fmt LtePhySubpktFmt_v1_Scmr_v35[] = {
    ...
    {PLACEHOLDER, "RSRP",
     0}, // skip 12 bits, then 12 bits, (0.0625 * (x + 640) - 180) dB
    {UINT, "Filtered RSRP",
     4}, // skip 12 bits, then 12 bits (0.0625 * x - 180) dBm
    {UINT, "RSRQ Rx[0]", 4}, // 10 bits, (0.0625 * x - 30) dB
    {PLACEHOLDER, "RSRQ Rx[1]",
     0},                     // skip 20 bits, then 10 bits, (0.0625 * x - 30) dB
    {UINT, "RSRQ Rx[2]", 4}, // skip 10 bits, then 10 bits, (0.0625 * x - 30) dB
    {PLACEHOLDER, "RSRQ Rx[3]",
     0},               // skip 20 bits, then 10 bits, (0.0625 * x - 30) dB
    {UINT, "RSRQ", 4}, // 10 bits, (0.0625 * x - 30) dB
    {PLACEHOLDER, "Filtered RSRQ",
     0},                     // skip 20 bits, then 12 bits, (0.0625 * x - 30) dB
    {UINT, "RSSI Rx[0]", 4}, // 11 bits (0.0625 * x - 110) dBm
    {PLACEHOLDER, "RSSI Rx[1]",
     0}, // skip 11 bits, then 11 bits (0.0625 * x - 110) dBm
    {UINT, "RSSI Rx[2]", 4}, // 11 bits (0.0625 * x - 110) dBm
    {PLACEHOLDER, "RSSI Rx[3]",
     0},               // skip 11 bits, then 11 bits (0.0625 * x - 110) dBm
    {UINT, "RSSI", 4}, // 11 bits (0.0625 * x - 110) dBm
    {SKIP, NULL, 20},
    {UINT, "FTL SNR Rx[0]", 4}, // 9 bits
...
const Fmt LtePhyIratCDMACellPilotFmt[] = {
    {UINT, "Pilot ID", 2}, {UINT, "RSS (dB)", 2}, {SKIP, NULL, 4},
    {UINT, "EcNo", 2},     {SKIP, NULL, 6},
};
...
// ------------------------------------------------------------
const Fmt LteRrcMibMessageLogPacketFmt[] = {{UINT, "Version", 1}};
...
// ----------------------------------------------------------------------------
// Haotian
const Fmt LtePdcpDlSrbIntegrityDataPduFmt[] = {{UINT, "Version", 1},
                                               {UINT, "Num SubPkt", 1},
                                               {SKIP, NULL, 44},
...
// ----------------------------------------------------------------------------
// Haotian
const Fmt LtePdcpUlSrbIntegrityDataPduFmt[] = {{UINT, "Version", 1},
                                               {UINT, "Num SubPkt", 1},
                                               {SKIP, NULL, 44},
                                               {UINT, "PDU Size", 2}, // 47-48
                                               {SKIP, NULL, 12}};
...
const ValueName LteMacConfigurationSubpkt_SubpktType[] = {
    {0, "Config Type Subpacket"},       {1, "DL Config SubPacket"},
    {2, "UL Config SubPacket"},         {3, "RACH Config SubPacket"},
    {4, "LC Config SubPacket"},         {7, "DL Transport Block"},
    {8, "UL Transport Block"},          {10, "UL Buffer Status SubPacket"},
    {11, "UL Tx Statistics SubPacket"}, {13, "eMBMS Config SubPacket"},
    {14, "All Rach Config SubPacket"},  {18, "ELS SubPacket"}};

const Fmt LteMacConfigurationFmt[] = {
    {UINT, "Version", 1}, {UINT, "Num SubPkt", 1}, {SKIP, NULL, 2}};

const Fmt LteMacConfiguration_SubpktHeader[] = {{UINT, "SubPacket ID", 1},
                                                {UINT, "Version", 1},
                                                {UINT, "SubPacket Size", 2}};

const Fmt LteMacConfigurationSubpkt_ConfigType[] = {{UINT, "Config reason", 4}};
...
const Fmt LteMacConfigurationSubpkt_eMBMSConfig[] = {
    {UINT, "Num eMBMS Active LCs",
     2}, // Not sure if this offset and length of this field is correct
    {SKIP, "NULL", 98}};
...
const ValueName BSRTrig[] = {
    {0, "No BSR"}, {1, "Cancelled"}, {2, "L-BSR"},
    {3, "S-BSR"},  {4, "Pad L-BSR"}, {5, "Pad S-BSR"},
};
...
const Fmt LteMacULTransportBlock_SubpktV1_SampleFmt[] = {
    {UINT, "HARQ ID", 1},    {UINT, "RNTI Type", 1},
    {UINT, "Sub-FN", 2}, // 4 bits
    {PLACEHOLDER, "SFN", 0}, {UINT, "Grant (bytes)", 2},
    {UINT, "RLC PDUs", 1},   {UINT, "Padding (bytes)", 2},
    {UINT, "BSR event", 1},  {UINT, "BSR trig", 1},
    {UINT, "HDR LEN", 1},
    // Mac Hdr + CE and UL TB Other Structure
};

const Fmt LteMacULTransportBlock_SubpktV2_SampleFmt[] = {
    {UINT, "Sub Id", 1},     {UINT, "Cell Id", 1},
    {UINT, "HARQ ID", 1},    {UINT, "RNTI Type", 1},
    {UINT, "Sub-FN", 2}, // 4 bits
    {PLACEHOLDER, "SFN", 0}, {UINT, "Grant (bytes)", 2},
    {UINT, "RLC PDUs", 1},   {UINT, "Padding (bytes)", 2},
...
// Jie
const ValueName RNTIType[] = {{0, "C-RNTI"},
                              {2, "P-RNTI"},
                              {3, "RA-RNTI"},
                              {4, "Temporary-C-RNTI"},
                              {5, "SI-RNTI"}};
...
const Fmt LteMacDLTransportBlock_SubpktV2_SampleFmt[] = {
    {UINT, "Sub-FN", 2},          {PLACEHOLDER, "SFN", 0},
    {UINT, "RNTI Type", 1},       {UINT, "HARQ ID", 1},
    {PLACEHOLDER, "Area ID", 0},  {UINT, "PMCH ID", 2},
    {UINT, "DL TBS (bytes)", 2},  {UINT, "RLC PDUs", 1},
    {UINT, "Padding (bytes)", 2}, {UINT, "HDR LEN", 1},
    // Mac Hdr + CE and UL TB Other Structure
};
...
const Fmt LteMacULTxStatistics_ULTxStatsSubPacketFmt[] = {
    {UINT, "Number of samples", 1},     {UINT, "Number of padding BSR", 1},
    {UINT, "Number of regular BSR", 1}, {UINT, "Number of periodic BSR", 1},
    {UINT, "Number of cancel BSR", 1},  {UINT, "Grant received", 4},
    {UINT, "Grant utilized", 4},        {SKIP, NULL, 3}};
...
const Fmt LteMacULBufferStatusInternal_ULBufferStatusSubPacket_LCIDFmt[] = {
    {UINT, "Ld Id", 1},      {UINT, "Priority", 1},   {UINT, "New bytes", 4},
    {UINT, "Retx bytes", 4}, {UINT, "Ctrl bytes", 2},
};
...
const Fmt LteRlcUlConfigLogPacketFmt[] = {
    {UINT, "Version", 1}, {UINT, "Num SubPkt", 1}, {SKIP, NULL, 2}, // reserved
};
...
const Fmt LteRlcDlAmAllPdu_Subpkt_PDU_Basic[] = {
    {PLACEHOLDER, "PDU TYPE", 0}, {PLACEHOLDER, "rb_cfg_idx", 0},
    {PLACEHOLDER, "Status", 0},   {UINT, "sys_fn", 2},
    {PLACEHOLDER, "sub_fn", 0},   {UINT, "pdu_bytes", 2},
    {UINT, "logged_bytes", 2},    {SKIP, NULL, 1},
    {UINT, "D/C LookAhead", 1},   {UINT, "SN", 1},
    // for control: cpt = STATUS(0)
    // for data: RF, P, FI, E
};
...
const Fmt LteMacRachTrigger_RachConfigSubpktPayload[] = {
    // Version 2
    {WCDMA_MEAS, "Preamble initial power (dB)",
     1}, // Note sure if it is correct
    {SKIP, NULL, 1},
...
const Fmt LteMacRachTrigger_RachReasonSubpktPayload[] = {
    // Version 1
    {UINT, "Rach reason", 1},       {PLACEHOLDER, "RACH Contention", 0},
    {BYTE_STREAM, "Maching ID", 6}, {SKIP, NULL, 1},
    {UINT, "Preamble", 1},          {BYTE_STREAM, "Preamble RA mask", 1},
    {UINT, "Msg3 size", 1},         {UINT, "Group chosen", 1},
    {UINT, "Radio condn (dB)", 1},  {BYTE_STREAM, "CRNTI", 2},
};
...
const Fmt LtePdcpDlConfig_Subpkt_ActiveRB_Fmt[] = {
    {UINT, "RB ID", 1},         {UINT, "RB-Cfg Idx", 1},
    {UINT, "EPS ID", 1},        {UINT, "RB mode", 1},
    {UINT, "RB type", 1},       {UINT, "SN length", 1},
    {UINT, "Status report", 1}, {UINT, "RoHC Max CID", 1},
    {UINT, "RoHC Enabled", 1},  {BYTE_STREAM, "RoHC Mask", 4},
};
...
const Fmt LtePdcpUlConfig_Subpkt_ActiveRB_Fmt[] = {
    ...
    {UINT, "EPS ID", 1},        {UINT, "RB mode", 1},
    {UINT, "RB type", 1},       {UINT, "SN length", 1},
    {UINT, "Discard timer", 2}, {UINT, "RoHC Max CID", 1},
    {UINT, "RoHC Enabled", 1},  {BYTE_STREAM, "RoHC Mask", 4},
};
...
const ValueName LtePucchPowerControl_Record_v4_PUCCH_Format[] = {
    {0, "Format 1"},  {1, "Format 1A"}, {2, "Format 1B"}, {3, "Format 2"},
    {4, "Format 2A"}, {5, "Format 2B"}, {6, "Format 3"},
};
...
const ValueName LtePucchPowerControl_Record_v24_PUCCH_Format[] = {
    {0, "Format 1"},    {1, "Format 1A"}, {2, "Format 1B"},
...
const Fmt LtePdcchPhichIndicationReport_Record_v5_p2[] = {
    // PDCCH Info
    {UINT, "Serv Cell Idx", 2},       {PLACEHOLDER, "RNTI Type", 0},
    {PLACEHOLDER, "Payload Size", 0}, {PLACEHOLDER, "Aggregation Level", 0},
    {UINT, "Search Space", 2},        {PLACEHOLDER, "SPS Grant Type", 0},
    {PLACEHOLDER, "New DL Tx", 0},    {PLACEHOLDER, "Num DL Trblks", 0},
};
...
const ValueName ValueNameRNTIType[] = {
    // 4 bits
    {0, "C-RNTI"},         {1, "SPS-RNTI"},         {2, "P-RNTI"},
    {3, "RA-RNTI"},        {4, "Temporary-C-RNTI"}, {5, "SI-RNTI"},
    {6, "TPC-PUSCH-RNTI"}, {7, "TPC-PUCCH-RNTI"},   {8, "MBMS RNTI"},
};
const ValueName ValueNameDCIFormat[] = {
    // 4 bits
    // Release 8
    // http://www.sharetechnote.com/html/LTE_Advanced_DCI.html
    {0, "Format 0"},  {1, "Format 1"},  {2, "Format 1A"}, {3, "Format 1B"},
    {4, "Format 1C"}, {5, "Format 1D"}, {6, "Format 2"},  {7, "Format 2A"},
    {8, "Format 3"},  {9, "Format 3A"},
};
...
const ValueName ValueNameCSFTxMode[] = {
    {0, "TM_Invalid"},      {1, "TM_Single_Ant_Port_0"},
    {2, "TM_TD_Rank_1"},    {3, "TM_OL_SM"},
    {4, "TM_CL_SM"},        {5, "TM_MU_MIMO"},
    {6, "TM_CL_Rank_1_PC"}, {7, "TM_Single_Ant_Port_5"},
};
...
const ValueName ValueNamePuschReportingMode[] = {
    {0, "MODE_APERIODIC_RM12"}, {1, "MODE_APERIODIC_RM20"},
    {2, "MODE_APERIODIC_RM22"}, {3, "MODE_APERIODIC_RM30"},
...
const ValueName ValueNameCDRXEvent[] = {
    {0, "ON_DURATION_TIMER_START"},  {1, "ON_DURATION_TIMER_END"},
    {2, "SHORT_CYCLE_START"},        {3, "SHORT_CYCLE_END"},
    {4, "LONG_CYCLE_START"},         {5, "LONG_CYCLE_END"},
    {6, "UL_RETX_TIMER_START"},      {7, "UL_RETX_TIMER_END"},
    {8, "INACTIVITY_TIMER_START"},   {9, "INACTIVITY_TIMER_END"},
    {10, "DL_DRX_RETX_TIMER_START"}, {11, "DL_DRX_RETX_TIMER_END"},
    {12, "CDRX_ON_2_OFF"},           {13, "CDRX_OFF_2_ON"},
};

const ValueName ValueNameWcdmaRrcStates[] = {
    {0, "DISCONNECTED"}, {1, "CONNECTING"}, {2, "CELL_FACH"},
    {3, "CELL_DCH"},     {4, "CELL_PCH"},   {5, "URA_PCH"},
};
...
const ValueName ValueNamePilotSet[] = {
    {0, "PreCandidate Set"}, {1, "Active Set"},    {2, "Candidate Set"},
    {4, "Neighbor Set"},     {8, "Remaining Set"},
};
...
const ValueName ValueNameGSMSignalingMessageType[] = {
    {0, "System Information Type 13"}, {7, "System Information Type 2quater"},
    {13, "Channel Release"},           {21, "Measurement Report"},
    {22, "Classmark Change"},          {25, "System Information Type 1"},
    {26, "System Information Type 2"}, {27, "System Information Type 3"},
    {28, "System Information Type 4"}, {29, "System Information Type 5"},
...
bool is_debug_packet(
    const char *b, size_t length); // Yuanjie: test if it's a debugging message
