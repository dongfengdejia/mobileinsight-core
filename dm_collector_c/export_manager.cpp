...
static int get_log_type(const char *b, size_t length) {
    int type_id = -1;
...
bool manager_export_binary(struct ExportManagerState *pstate, const char *b,
                           size_t length) {
    ...
    if (pstate->whitelist.count(type_id) > 0) { // filter
        ...
        if (pstate->log_fp != NULL) {
            ...
            size_t cnt = fwrite(frame.c_str(), sizeof(char), frame.size(),
                                pstate->log_fp);
            (void)cnt;
        }
        return true;
...
void manager_change_config(struct ExportManagerState *pstate,
    ...
    if (pstate->log_fp != NULL && new_path != NULL &&
        pstate->filename != new_path) { // close old file
        fclose(pstate->log_fp);
        pstate->log_fp = NULL;
        pstate->filename = "";
    }
    if (pstate->log_fp == NULL &&
        new_path != NULL) { // open new file if necessary
