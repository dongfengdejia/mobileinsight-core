...
static int _decode_lte_phy_bplmn_cell_confirm_payload(const char *b, int offset,
                                                      size_t length,
                                                      json &result) {
    int start = offset;
    ...
    case 4: {
        offset +=
            _decode_by_fmt(LtePhyBplmnCellConfirm_Payload_v4,
                           ARRAY_SIZE(LtePhyBplmnCellConfirm_Payload_v4, Fmt),
                           b, offset, length, result);
        int iStandrdsVersion = _search_result_int(result, "Standards Version");
        (void)_map_result_field_to_name(
            result, "Standards Version",
        ...
        unsigned int iNonDecodeP1 =
            _search_result_uint(result, "SRX Lev Calculated");
        int iSRXLevCalculated = iNonDecodeP1 & 1;  // last 1 bits
        int iSRXLev = (iNonDecodeP1 >> 1) & 65535; // next 16 bits
        _replace_result_int(result, "SRX Lev Calculated", iSRXLevCalculated);
        _replace_result_int(result, "SRX Lev", iSRXLev);
        (void)_map_result_field_to_name(
            result, "SRX Lev Calculated",
        ...
        if (iStandrdsVersion == 1) {
            offset +=
                _decode_by_fmt(LtePhyBplmnCellConfirm_Rel9Info,
                               ARRAY_SIZE(LtePhyBplmnCellConfirm_Rel9Info, Fmt),
                               b, offset, length, result);
            unsigned int iNonDecodeP2 =
                _search_result_uint(result, "Rel 9 Info S Qual Calculated");
            int iR9SQCalculated = iNonDecodeP2 & 1;     // last 1 bit
            int iR9SQual = (iNonDecodeP2 >> 1) & 65535; // next 16 bits
            _replace_result_int(result, "Rel 9 Info S Qual Calculated",
                                iR9SQCalculated);
            _replace_result_int(result, "Rel 9 Info S Qual", iR9SQual);
            (void)_map_result_field_to_name(
                result, "Rel 9 Info S Qual Calculated",
