...
const Fmt LtePhyPuschTxReport_Record_v43[] = {
    ...
    {SKIP, NULL, 4}, // Num DL carriers, Ack Nak Index, Ack Nak Late, Csf Late,
                     // Dropped Pusch
...
static int _decode_lte_phy_pusch_tx_report_payload(const char *b, int offset,
                                                   size_t length,
                                                   json &result) {
    int start = offset;
    ...
    case 43: {
        offset +=
            _decode_by_fmt(LtePhyPuschTxReport_Payload_v43,
                           ARRAY_SIZE(LtePhyPuschTxReport_Payload_v43, Fmt), b,
        ...
        for (int i = 0; i < num_record; i++) {
            ...
            offset +=
                _decode_by_fmt(LtePhyPuschTxReport_Record_v43,
            ...
            pyfloat = fCodingRateData;
            _replace_result(result_record_item, "Coding Rate Data", pyfloat);
            ...
            _replace_result_int(result_record_item, "ACK", iAck);
            (void)_map_result_field_to_name(
                result_record_item, "ACK", ValueNameExistsOrNone,
                ARRAY_SIZE(ValueNameExistsOrNone, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "CQI", iCQI);
            (void)_map_result_field_to_name(
                result_record_item, "CQI", ValueNameExistsOrNone,
                ARRAY_SIZE(ValueNameExistsOrNone, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "RI", iRI);
            (void)_map_result_field_to_name(
                result_record_item, "RI", ValueNameExistsOrNone,
                ARRAY_SIZE(ValueNameExistsOrNone, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "Frequency Hopping",
                                iFrequencyHopping);
            (void)_map_result_field_to_name(
                result_record_item, "Frequency Hopping",
                ValueNameEnableOrDisable,
                ARRAY_SIZE(ValueNameEnableOrDisable, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "Redund Ver", iRedundVer);
            _replace_result_int(result_record_item, "Mirror Hopping",
                                iMirrorHopping);
            ...
            _replace_result_int(result_record_item,
                                ...
                                iCSDSS1);
            _replace_result_int(result_record_item, "DMRS Root Slot 0",
                                iDMRSRootSlot0);
            _replace_result_int(result_record_item, "UE SRS", iUESRS);
            (void)_map_result_field_to_name(
                result_record_item, "UE SRS", ValueNameOnOrOff,
                ARRAY_SIZE(ValueNameOnOrOff, ValueName), "(MI)Unknown");

            u_temp =
                _search_result_uint(result_record_item, "DMRS Root Slot 1");
            ...
            _replace_result_int(result_record_item, "DMRS Root Slot 1",
                                iDMRSRootSlot1);
            _replace_result_int(result_record_item, "Start RB Slot 0",
                                iStartRBSlot0);
            _replace_result_int(result_record_item, "Start RB Slot 1",
                                iStartRBSlot1);
            _replace_result_int(result_record_item, "Num of RB", iNumOfRB);
            ...
            strAckPayload += hex;
            pystr = strAckPayload.c_str();
            _replace_result(result_record_item, "ACK Payload", pystr);
            int iAckNakInpLength0 = (u_temp >> 20) & 15;
            int iAckNakInpLength1 = (u_temp >> 24) & 15;
            int iNumRiBitsNri = (u_temp >> 29) & 7;
            _replace_result_int(result_record_item, "ACK/NAK Inp Length 0",
                                iAckNakInpLength0);
            _replace_result_int(result_record_item, "ACK/NAK Inp Length 1",
                                iAckNakInpLength1);
            _replace_result_int(result_record_item, "Num RI Bits NRI (bits)",
                                iNumRiBitsNri);
            ...
            strRiPayload += hex;
            pystr = strRiPayload.c_str();
            _replace_result(result_record_item, "RI Payload", pystr);
            _replace_result_int(result_record_item, "Rate Matched RI Bits",
                                iRateMatchedRiBits);
            _replace_result_int(result_record_item, "PUSCH Mod Order",
                                iPuschModOrder);
            (void)_map_result_field_to_name(
                result_record_item, "PUSCH Mod Order", ValueNameModulation,
                ARRAY_SIZE(ValueNameModulation, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "PUSCH Digital Gain (dB)",
                                iPuschDigitalGain);
            _replace_result_int(result_record_item, "SRS Occasion",
                                iSrsOccasion);
            (void)_map_result_field_to_name(
                result_record_item, "SRS Occasion", ValueNameOnOrOff,
                ARRAY_SIZE(ValueNameOnOrOff, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "Re-tx Index", iRetxIndex);
            (void)_map_result_field_to_name(
                result_record_item, "Re-tx Index", ValueNameNumber,
                ARRAY_SIZE(ValueNameNumber, ValueName), "(MI)Unknown");

            u_temp =
                _search_result_uint(result_record_item, "PUSCH Tx Power (dBm)");
            ...
            _replace_result_int(result_record_item, "PUSCH Tx Power (dBm)",
                                iPuschTxPower);
            _replace_result_int(result_record_item, "Num CQI Bits",
                                iNumCqiBits);
            _replace_result_int(result_record_item, "Rate Matched CQI Bits",
                                iRateMatchedCqiBits);
    ...
    case 102: {
        offset +=
            _decode_by_fmt(LtePhyPuschTxReport_Payload_v102,
                           ARRAY_SIZE(LtePhyPuschTxReport_Payload_v102, Fmt), b,
                           offset, length, result);
        temp = _search_result_int(result, "Serving Cell ID");
        int iServingCellId = temp & 511;   // 9 bits
        int num_record = (temp >> 9) & 31; // 5 bits
        ...
        for (int i = 0; i < num_record; i++) {
            json result_record_item;
            offset +=
                _decode_by_fmt(LtePhyPuschTxReport_Record_v102,
            ...
            _replace_result_int(result_record_item, "Carrier Index",
                                iCarrierIndex);
            (void)_map_result_field_to_name(
                result_record_item, "Carrier Index", ValueNameCarrierIndex,
            ...
            _replace_result_int(result_record_item, "Resource Allocation Type",
                                iResourceAllocationType);
            _replace_result_int(result_record_item, "Start RB Slot 0",
                                iStartRBSlot0);
            _replace_result_int(result_record_item, "Start RB Slot 1",
                                iStartRBSlot1);
            ...
            pyfloat = fCodingRate;
            _replace_result(result_record_item, "Coding Rate", pyfloat);
            ...
            _replace_result_int(result_record_item, "Rate Matched ACK Bits",
                                iRateMatchedAckBits);
            char hex[10] = {};
            std::string strRiPayload = "0x";
            ...
            u_temp = _search_result_uint(result_record_item, "PUSCH Mod Order");
            ...
            _replace_result_int(result_record_item, "PUSCH Digital Gain (dB)",
                                iPuschDigitalGain);
            _replace_result_int(result_record_item, "Start RB Cluster1",
                                iStartRBCluster1);
            _replace_result_int(result_record_item, "Num RB Cluster1",
                                iNumRBCluster1);
            ...
            _replace_result_int(result_record_item, "Num DL Carriers",
                                iNumDLCarriers);
            _replace_result_int(result_record_item, "Ack Nack Index",
                                iAckNackIndex);
            _replace_result_int(result_record_item, "Ack Nack Late",
                                iAckNackLate);
            _replace_result_int(result_record_item, "CSF Late", iCSFLate);
            _replace_result_int(result_record_item, "Drop PUSCH", iDropPusch);

            u_temp = _search_result_uint(
                result_record_item,
                "Cyclic Shift of DMRS Symbols Slot 0 (Samples)");
    ...
    case 124: {
        ...
        for (int i = 0; i < num_record; i++) {
            json result_record_item;
            offset +=
                _decode_by_fmt(LtePhyPuschTxReport_Record_v124,
                               ARRAY_SIZE(LtePhyPuschTxReport_Record_v124, Fmt),
                               b, offset, length, result_record_item);
            u_temp =
                _search_result_uint(result_record_item, "UL Carrier Index");
            int iCarrierIndex = u_temp & 3;            // 2 bits
            int iAck = (u_temp >> 2) & 1;              // 1 bit
            int iCQI = (u_temp >> 3) & 1;              // 1 bit
            int iRI = (u_temp >> 4) & 1;               // 1 bit
            ...
            int iRedundVer = (u_temp >> 12) & 3;       // 2 bits
            int iMirrorHopping = (u_temp >> 14) & 3;   // 2 bits
            _replace_result_int(result_record_item, "UL Carrier Index",
                                iCarrierIndex);
            (void)_map_result_field_to_name(
                result_record_item, "UL Carrier Index", ValueNameCarrierIndex,
                ARRAY_SIZE(ValueNameCarrierIndex, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "ACK", iAck);
            (void)_map_result_field_to_name(
                result_record_item, "ACK", ValueNameExistsOrNone,
            ...
            _replace_result_int(result_record_item, "DL Carrier Index",
                                iDlCarrierIndex);
            (void)_map_result_field_to_name(
                result_record_item, "DL Carrier Index", ValueNameCarrierIndex,
                ARRAY_SIZE(ValueNameCarrierIndex, ValueName), "(MI)Unknown");
