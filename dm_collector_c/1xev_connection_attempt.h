...
static int _decode_1xev_connection_attempt_payload(const char *b, int offset,
                                                   size_t length,
                                                   json &result) {
    ...
    _replace_result_int(result, "Conn Attempt Outcome", iOutcome);
    (void)_map_result_field_to_name(
        result, "Conn Attempt Outcome", _1xEVConnectionAttempt_Outcome,
        ARRAY_SIZE(_1xEVConnectionAttempt_Outcome, ValueName), "(MI)Unknown");
    _replace_result_int(result, "Stat of TCA/RTCACK Msgs", iStat);
    (void)_map_result_field_to_name(
        result, "Stat of TCA?RTCACK Msgs", _1xEVConnectionAttempt_Stat,
        ARRAY_SIZE(_1xEVConnectionAttempt_Stat, ValueName), "(MI)Unknown");
    return 0;
}
