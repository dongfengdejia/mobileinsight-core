...
static UINT16 calc_crc(UINT8 *data, size_t len, UINT16 crc) {
...
void feed_binary(const char *b, int length) { buffer.append(b, length); }

void reset_binary() { buffer.clear(); }

static void unescape(std::string &frame) {
    bool esc = false;
    std::string output;
...
bool get_next_frame(std::string &output_frame, bool &crc_correct) {
    ...
    UINT16 crc16 =
        calc_crc((UINT8 *)output_frame.c_str(), output_frame.size(), 0);
