...
const Fmt LtePhyPucchTxReport_Record_p1_v101[] = {
    ...
    {PLACEHOLDER, "Format",
     0}, // 3 bits, LtePucchPowerControl_Record_v4_PUCCH_Format
    {PLACEHOLDER, "Start RB Slot 0", 0},           // 7 bits
    {PLACEHOLDER, "Start RB Slot 1", 0},           // 7 bits
    {PLACEHOLDER, "SRS Shorting for 2nd Slot", 0}, // 1 bit
    {PLACEHOLDER, "UE SRS", 0},                    // 1 bit
...
static int _decode_lte_phy_pucch_tx_report_payload(const char *b, int offset,
                                                   size_t length,
                                                   json &result) {
    int start = offset;
    ...
    case 21: {
        offset +=
            _decode_by_fmt(LtePhyPucchTxReport_Payload_v21,
                           ARRAY_SIZE(LtePhyPucchTxReport_Payload_v21, Fmt), b,
        ...
        for (int i = 0; i < num_record; i++) {
            ...
            offset += _decode_by_fmt(
                LtePhyPucchTxReport_Record_p1_v21,
                ARRAY_SIZE(LtePhyPucchTxReport_Record_p1_v21, Fmt), b, offset,
                length, result_record_item);

            temp = _search_result_int(result_record_item, "CQI Payload");
            std::string strCQIPayload = (std::bitset<16>(temp)).to_string();
            pystr = strCQIPayload.c_str();
            _replace_result(result_record_item, "CQI Payload", pystr);
            ...
            _replace_result_int(result_record_item, "Format", iFormat);
            (void)_map_result_field_to_name(
                result_record_item, "Format",
            ...
            _replace_result_int(result_record_item, "Start RB Slot 1",
                                iStartRBSlot1);
            _replace_result_int(result_record_item, "SRS Shorting for 2nd Slot",
                                iSRSShortingfor2ndSlot);
            (void)_map_result_field_to_name(
                result_record_item, "SRS Shorting for 2nd Slot",
                ValueNameSRSShortingfor2ndSlot,
                ARRAY_SIZE(ValueNameSRSShortingfor2ndSlot, ValueName),
                "(MI)Unknown");
            ...
            _replace_result_int(result_record_item, "DMRS Seq Slot 0",
                                iDMRSSeqSlot0);
            _replace_result_int(result_record_item, "DMRS Seq Slot 1",
                                iDMRSSeqSlot1);
            ...
            json result_record_css;
            for (int j = 0; j < 14; j++) {
                json result_record_css_item;
                offset += _decode_by_fmt(
                    LtePhyPucchTxReport_Record_CyclicShift_v21,
                    ARRAY_SIZE(LtePhyPucchTxReport_Record_CyclicShift_v21, Fmt),
                    b, offset, length, result_record_css_item);

                result_record_css.push_back(result_record_css_item);
            }

            result_record_item["Cyclic Shift Seq"] = result_record_css;

            offset += _decode_by_fmt(
                LtePhyPucchTxReport_Record_p2_v21,
                ARRAY_SIZE(LtePhyPucchTxReport_Record_p2_v21, Fmt), b, offset,
                length, result_record_item);

            temp =
                _search_result_int(result_record_item, "PUCCH Tx Power (dBm)");
            int iPucchTxPower = temp & 63; // 6 bits
            _replace_result_int(result_record_item, "PUCCH Tx Power (dBm)",
                                iPucchTxPower);

            u_temp = _search_result_uint(result_record_item, "Tx Resampler");
            double fTxResampler = u_temp / 4294967295.0;
            pydouble = fTxResampler;
            _replace_result(result_record_item, "Tx Resampler", pydouble);
    ...
    case 43: {
        offset +=
            _decode_by_fmt(LtePhyPucchTxReport_Payload_v43,
                           ARRAY_SIZE(LtePhyPucchTxReport_Payload_v43, Fmt), b,
                           offset, length, result);
        ...
        for (int i = 0; i < num_record; i++) {
            json result_record_item;
            offset += _decode_by_fmt(
                LtePhyPucchTxReport_Record_p1_v43,
                ARRAY_SIZE(LtePhyPucchTxReport_Record_p1_v43, Fmt), b, offset,
                length, result_record_item);
            ...
            for (int j = 0; j < 14; j++) {
                ...
                offset += _decode_by_fmt(
                    ...
                    ARRAY_SIZE(LtePhyPucchTxReport_Record_CyclicShift_v43, Fmt),
                    b, offset, length, result_record_css_item);
            ...
            offset += _decode_by_fmt(
                LtePhyPucchTxReport_Record_p2_v43,
                ARRAY_SIZE(LtePhyPucchTxReport_Record_p2_v43, Fmt), b, offset,
                length, result_record_item);
            ...
            pystr = strAckPayload.c_str();
            _replace_result(result_record_item, "ACK Payload", pystr);
            _replace_result_int(result_record_item, "ACK Payload Length",
                                iAckPayloadLength);
            _replace_result_int(result_record_item, "SR Bit FMT3", iSRBitFMT3);
            _replace_result_int(result_record_item, "Num DL Carriers",
                                iNumDLCarriers);

            temp = _search_result_int(result_record_item, "Ack Nak Index");
            ...
            for (unsigned int idx = 0; idx < 4 - hexAckNakIndex.length();
                 idx++) {
                strAckNakIndex += "0";
            }
            strAckNakIndex += hexAckNakIndex;
            pystr = strAckNakIndex.c_str();
            _replace_result(result_record_item, "Ack Nak Index", pystr);
            _replace_result_int(result_record_item, "Ack Nak Late",
                                iAckNakLate);
            _replace_result_int(result_record_item, "Csf Late", iCsfLate);
            _replace_result_int(result_record_item, "Dropped Pucch",
                                iDroppedPucch);
    ...
    case 101: {
        offset +=
            _decode_by_fmt(LtePhyPucchTxReport_Payload_v101,
                           ARRAY_SIZE(LtePhyPucchTxReport_Payload_v101, Fmt), b,
                           offset, length, result);
        ...
        for (int i = 0; i < num_record; i++) {
            json result_record_item;
            offset += _decode_by_fmt(
                LtePhyPucchTxReport_Record_p1_v101,
                ARRAY_SIZE(LtePhyPucchTxReport_Record_p1_v101, Fmt), b, offset,
                length, result_record_item);
            ...
            for (int j = 0; j < 14; j++) {
                ...
                offset += _decode_by_fmt(
                    ...
                    ARRAY_SIZE(LtePhyPucchTxReport_Record_CyclicShift_v101,
                               Fmt),
                    b, offset, length, result_record_css_item);
            ...
            offset += _decode_by_fmt(
                LtePhyPucchTxReport_Record_p2_v101,
                ARRAY_SIZE(LtePhyPucchTxReport_Record_p2_v101, Fmt), b, offset,
                length, result_record_item);
    ...
    case 121: {
        offset +=
            _decode_by_fmt(LtePhyPucchTxReport_Payload_v121,
                           ARRAY_SIZE(LtePhyPucchTxReport_Payload_v121, Fmt), b,
                           offset, length, result);
        ...
        for (int i = 0; i < num_record; i++) {
            json result_record_item;
            offset += _decode_by_fmt(
                LtePhyPucchTxReport_Record_p1_v121,
                ARRAY_SIZE(LtePhyPucchTxReport_Record_p1_v121, Fmt), b, offset,
                length, result_record_item);
            ...
            for (int j = 0; j < 14; j++) {
                ...
                offset += _decode_by_fmt(
                    ...
                    ARRAY_SIZE(LtePhyPucchTxReport_Record_CyclicShift_v121,
                               Fmt),
                    b, offset, length, result_record_css_item);
            ...
            offset += _decode_by_fmt(
                LtePhyPucchTxReport_Record_p2_v121,
                ARRAY_SIZE(LtePhyPucchTxReport_Record_p2_v121, Fmt), b, offset,
                length, result_record_item);
