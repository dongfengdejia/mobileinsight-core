...
static int _decode_lte_phy_pdcch_decoding_result_payload(const char *b,
    ...
    case 21: {
        offset += _decode_by_fmt(
            LtePhyPdcchDecodingResult_Payload_v21,
            ARRAY_SIZE(LtePhyPdcchDecodingResult_Payload_v21, Fmt), b, offset,
            length, result);
        ...
        for (int i = 0; i < num_record; i++) {
            ...
            offset += _decode_by_fmt(
                LtePhyPdcchDecodingResult_Hypothesis_v21,
                ARRAY_SIZE(LtePhyPdcchDecodingResult_Hypothesis_v21, Fmt), b,
                offset, length, result_record_item);
            unsigned int iNonDecodeP3 =
                _search_result_uint(result_record_item, "Aggregation Level");
            ...
            _replace_result_int(result_record_item, "Aggregation Level",
                                iAggLv);
            (void)_map_result_field_to_name(
                result_record_item, "Aggregation Level",
                ValueNameAggregationLevel,
                ARRAY_SIZE(ValueNameAggregationLevel, ValueName),
                "(MI)Unknown");
            _replace_result_int(result_record_item, "Candidate", iCandidate);
            _replace_result_int(result_record_item, "Search Space Type",
                                iSearchSpaceType);
            (void)_map_result_field_to_name(
                result_record_item, "Search Space Type",
                ValueNameSearchSpaceType,
                ARRAY_SIZE(ValueNameSearchSpaceType, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "DCI Format", iDCIFormat);
            (void)_map_result_field_to_name(
                result_record_item, "DCI Format", ValueNameDCIFormat,
                ARRAY_SIZE(ValueNameDCIFormat, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "Decode Status",
                                iDecodeStatus);
            (void)_map_result_field_to_name(
                result_record_item, "Decode Status", ValueNameRNTIType,
                ARRAY_SIZE(ValueNameRNTIType, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "Start CCE", iStartCCE);
            _replace_result_int(result_record_item, "Payload Size",
                                iPayloadSize);
            _replace_result_int(result_record_item, "Tail Match", iTailMatch);
            (void)_map_result_field_to_name(
                result_record_item, "Tail Match", ValueNameMatchOrNot,
                ARRAY_SIZE(ValueNameMatchOrNot, ValueName), "(MI)Unknown");
            (void)_map_result_field_to_name(
                result_record_item, "Prune Status", ValueNamePruneStatus,
                ARRAY_SIZE(ValueNamePruneStatus, ValueName), "(MI)Unknown");
            unsigned int iNonDecode =
                _search_result_uint(result_record_item, "Norm Energy Metric");
            float fNEM = iNonDecode / 65535.0;
            pyfloat = fNEM;
            _replace_result(result_record_item, "Norm Energy Metric", pyfloat);
            iNonDecode =
                _search_result_uint(result_record_item, "Symbol Error Rate");
            float fSER = iNonDecode / 2147483648.0;
            pyfloat = fSER;
            _replace_result(result_record_item, "Symbol Error Rate", pyfloat);

            result_record.push_back(result_record_item);
        }
        result["Hypothesis"] = result_record;
        return offset - start;
    }
    case 42: {
        offset += _decode_by_fmt(
            LtePhyPdcchDecodingResult_Payload_v42,
            ARRAY_SIZE(LtePhyPdcchDecodingResult_Payload_v42, Fmt), b, offset,
            length, result);
        ...
        (void)_map_result_field_to_name(
            result, "CIF Configured", ValueNameTrueOrFalse,
            ARRAY_SIZE(ValueNameTrueOrFalse, ValueName), "(MI)Unknown");
        _replace_result_int(result, "Two bits CSI Configured", iTBCC);
        (void)_map_result_field_to_name(
            result, "Two bits CSI Configured", ValueNameTrueOrFalse,
            ARRAY_SIZE(ValueNameTrueOrFalse, ValueName), "(MI)Unknown");
        _replace_result_int(result, "Aperiodic SRS Configured", iASC);
        (void)_map_result_field_to_name(
            result, "Aperiodic SRS Configured", ValueNameTrueOrFalse,
            ARRAY_SIZE(ValueNameTrueOrFalse, ValueName), "(MI)Unknown");
        _replace_result_int(result, "Frame Structure", iFrameStructure);
        (void)_map_result_field_to_name(
            result, "Frame Structure", ValueNameFrameStructure,
            ARRAY_SIZE(ValueNameFrameStructure, ValueName), "(MI)Unknown");
        _replace_result_int(result, "Num eNB Antennas", iNumAntennas);
        (void)_map_result_field_to_name(
            result, "Num eNB Antennas", ValueNameNumNBAntennas,
            ARRAY_SIZE(ValueNameNumNBAntennas, ValueName), "(MI)Unknown");
        _replace_result_int(result, "DL CP", iDLCP);
        (void)_map_result_field_to_name(
            result, "DL CP", ValueNameNormalOrNot,
            ARRAY_SIZE(ValueNameNormalOrNot, ValueName), "(MI)Unknown");
        ...
        for (int i = 0; i < num_record; i++) {
            ...
            json result_record_item;
            offset += _decode_by_fmt(
                LtePhyPdcchDecodingResult_Hypothesis_v42,
                ARRAY_SIZE(LtePhyPdcchDecodingResult_Hypothesis_v42, Fmt), b,
                offset, length, result_record_item);
            temp = _search_result_uint(result_record_item, "Aggregation Level");
            ...
            temp = _search_result_uint(result_record_item, "Start CCE");
            int iStartCCE = (temp >> 1) & 127;  // 1 + 7 bits
            int iNZLC = (temp >> 8) & 511;      // 9 bits
            int iNormal = (temp >> 17) & 32767; // 15 bits
            temp = _search_result_uint(result_record_item, "Prune Status");
            ...
            (void)_map_result_field_to_name(
                result_record_item, "DCI Format", ValueNameDCIFormat,
                ARRAY_SIZE(ValueNameDCIFormat, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "Decode States",
                                iDecodeStatus);
            (void)_map_result_field_to_name(
                result_record_item, "Decode States", ValueNameRNTIType,
                ARRAY_SIZE(ValueNameRNTIType, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "Payload Size",
                                iPayloadSize);
            _replace_result_int(result_record_item, "Tail Match", iTailMatch);
            ...
            _replace_result_int(result_record_item, "Start CCE", iStartCCE);
            _replace_result_int(result_record_item, "Non Zero Llr Count",
                                iNZLC);
            _replace_result_int(result_record_item, "Normal", iNormal);
            _replace_result_int(result_record_item, "Prune Status",
                                iPruneStatus);
            (void)_map_result_field_to_name(
                result_record_item, "Prune Status", ValueNamePruneStatus,
                ARRAY_SIZE(ValueNamePruneStatus, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "Energy Metric",
                                iEnergyMetric);
            unsigned int iNonDecode =
                _search_result_uint(result_record_item, "Norm Energy Metric");
            float fNEM = iNonDecode / 65535.0;
    ...
    case 43: {
        offset += _decode_by_fmt(
            LtePhyPdcchDecodingResult_Payload_v43,
            ARRAY_SIZE(LtePhyPdcchDecodingResult_Payload_v43, Fmt), b, offset,
            length, result);
        ...
        for (int i = 0; i < num_record; i++) {
            json result_record_item;
            offset += _decode_by_fmt(
                LtePhyPdcchDecodingResult_Record_v43,
                ARRAY_SIZE(LtePhyPdcchDecodingResult_Record_v43, Fmt), b,
                offset, length, result_record_item);
            ...
            (void)_map_result_field_to_name(
                result_record_item, "CIF Configured", ValueNameTrueOrFalse,
                ARRAY_SIZE(ValueNameTrueOrFalse, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "Two bits CSI Configured",
                                iTBCC);
            (void)_map_result_field_to_name(
                result_record_item, "Two bits CSI Configured",
                ValueNameTrueOrFalse,
                ARRAY_SIZE(ValueNameTrueOrFalse, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "Aperiodic SRS Configured",
                                iASC);
            (void)_map_result_field_to_name(
                result_record_item, "Aperiodic SRS Configured",
                ValueNameTrueOrFalse,
                ARRAY_SIZE(ValueNameTrueOrFalse, ValueName), "(MI)Unknown");
            _replace_result_int(result_record_item, "Number of Hypothesis",
                                num_hypothesis);

            json result_record_hypothesis;
            for (int j = 0; j < num_hypothesis; j++) {
                json result_record_hypothesis_item;
                offset += _decode_by_fmt(
                    LtePhyPdcchDecodingResult_Hypothesis_v43,
                ...
                (void)_map_result_field_to_name(
                    result_record_hypothesis_item, "Aggregation Level",
                    ValueNameAggregationLevel,
                    ARRAY_SIZE(ValueNameAggregationLevel, ValueName),
                    "(MI)Unknown");
                _replace_result_int(result_record_hypothesis_item, "Candidate",
                                    iCandidate);
                _replace_result_int(result_record_hypothesis_item,
                                    "Search Space Type", iSearchSpaceType);
                (void)_map_result_field_to_name(
                    result_record_hypothesis_item, "Search Space Type",
                    ValueNameSearchSpaceType,
                    ARRAY_SIZE(ValueNameSearchSpaceType, ValueName),
                    "(MI)Unknown");
                _replace_result_int(result_record_hypothesis_item, "DCI Format",
                                    iDCIFormat);
                (void)_map_result_field_to_name(
                    result_record_hypothesis_item, "DCI Format",
                    ValueNameDCIFormat,
                    ARRAY_SIZE(ValueNameDCIFormat, ValueName), "(MI)Unknown");
                _replace_result_int(result_record_hypothesis_item,
                                    "Decode States", iDecodeStatus);
                (void)_map_result_field_to_name(
                    result_record_hypothesis_item, "Decode States",
                    ValueNameRNTIType, ARRAY_SIZE(ValueNameRNTIType, ValueName),
                    "(MI)Unknown");
                _replace_result_int(result_record_hypothesis_item,
                                    "Payload Size", iPayloadSize);
                _replace_result_int(result_record_hypothesis_item, "Tail Match",
                                    iTailMatch);
                (void)_map_result_field_to_name(
                    result_record_hypothesis_item, "Tail Match",
                    ValueNameMatchOrNot,
                    ARRAY_SIZE(ValueNameMatchOrNot, ValueName), "(MI)Unknown");
                _replace_result_int(result_record_hypothesis_item,
                                    "Non Zero Symbol Mismatch Count", iNZSMC);
                _replace_result_int(result_record_hypothesis_item, "Start CCE",
                                    iStartCCE);
                _replace_result_int(result_record_hypothesis_item,
                                    "Non Zero Llr Count", iNZLC);
                _replace_result_int(result_record_hypothesis_item, "Normal",
                                    iNormal);
                _replace_result_int(result_record_hypothesis_item,
                                    "Prune Status", iPruneStatus);
                (void)_map_result_field_to_name(
                    result_record_hypothesis_item, "Prune Status",
                    ValueNamePruneStatus,
                    ARRAY_SIZE(ValueNamePruneStatus, ValueName), "(MI)Unknown");
                _replace_result_int(result_record_hypothesis_item,
                                    "Energy Metric", iEnergyMetric);
                unsigned int iNonDecode = _search_result_uint(
                    result_record_hypothesis_item, "Norm Energy Metric");
                float fNEM = iNonDecode / 65535.0;
                pyfloat = fNEM;
                ...
                result_record_hypothesis.push_back(
                    result_record_hypothesis_item);
            }

            offset += (11 - num_hypothesis) * 28;
            result_record_item["Hypothesis"] = result_record_hypothesis;

            result_record.push_back(result_record_item);
        }
        result["SF"] = result_record;
        return offset - start;
    }
    case 101: {
        offset += _decode_by_fmt(
            LtePhyPdcchDecodingResult_Payload_v101,
            ARRAY_SIZE(LtePhyPdcchDecodingResult_Payload_v101, Fmt), b, offset,
            length, result);
        unsigned int temp = _search_result_uint(result, "Subframe Number");
        int iSubFN = temp & 15;            // 4 bits
        int iSysFN = (temp >> 4) & 1023;   // 10 bits
        ...
        for (int i = 0; i < num_record; i++) {
            ...
            json result_record_item;
            offset += _decode_by_fmt(
                LtePhyPdcchDecodingResult_Hypothesis_v101,
                ARRAY_SIZE(LtePhyPdcchDecodingResult_Hypothesis_v101, Fmt), b,
                offset, length, result_record_item);
            temp = _search_result_uint(result_record_item, "Aggregation Level");
    ...
    case 121: {
        offset += _decode_by_fmt(
            LtePhyPdcchDecodingResult_Payload_v121,
            ARRAY_SIZE(LtePhyPdcchDecodingResult_Payload_v121, Fmt), b, offset,
            length, result);
        unsigned int temp = _search_result_uint(result, "Subframe Number");
        int iSubFN = temp & 15;            // 4 bits
        int iSysFN = (temp >> 4) & 1023;   // 10 bits
        ...
        for (int i = 0; i < num_record; i++) {
            ...
            json result_record_item;
            offset += _decode_by_fmt(
                LtePhyPdcchDecodingResult_Hypothesis_v121,
                ARRAY_SIZE(LtePhyPdcchDecodingResult_Hypothesis_v121, Fmt), b,
                offset, length, result_record_item);
            temp = _search_result_uint(result_record_item, "Aggregation Level");
            ...
            _replace_result_int(result_record_item,
                                "Non Zero Symbol Mismatch Count", iNZSMC);
            _replace_result_int(result_record_item, "Start CCE", iStartCCE);
            _replace_result_int(result_record_item, "Alt TBS Enabled",
                                altTbsEnabled);
            _replace_result_int(result_record_item, "Non Zero Llr Count",
                                iNZLC);
            _replace_result_int(result_record_item, "Normal", iNormal);
