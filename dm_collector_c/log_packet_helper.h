...
#define _search_result_int(dict, key) (int)dict[(key)]
#define _search_result_uint(dict, key) (unsigned int)dict[(key)]
#define _replace_result_int(a, b, c) a[b] = c
#define _replace_result(a, b, c) a[b] = c

// Find a field by its name in a result list.
// Return: i or -1
// static int _find_result_index(PyObject *result, const char *target) {
...
// static PyObject *_search_result(PyObject *result, const char *target) {
...
// Return: int
// static int _search_result_int(PyObject *result, const char *target)
// __attribute__((unused)); static int _search_result_int(PyObject *result,
// const char *target) {
...
// Return: unsigned int
// static unsigned int _search_result_uint(PyObject *result, const char *target)
// __attribute__((unused)); static unsigned int _search_result_uint(PyObject
// *result, const char *target) {
//     PyObject *item = _search_result(result, target);
//     assert(PyInt_Check(item));
...
// static PyObject *_replace_result(PyObject *result, const char *target,
// PyObject *new_object) {
//     int i = _find_result_index(result, target);
//     if (i >= 0) {
//         PyObject *t = PySequence_GetItem(result, i);
...
// Return: New reference to the old object
// static PyObject *_replace_result_int(PyObject *result, const char *target,
// int new_int) __attribute__((unused)); static PyObject
// *_replace_result_int(PyObject *result, const char *target, int new_int) {
//     PyObject *pyint = Py_BuildValue("i", new_int);
//     _replace_result(result, target, pyint);
//     return old_object;
// }
...
// Return: old number
// static int _map_result_field_to_name(PyObject *result, const char *target,
// const ValueName mapping[], int n, const char *not_found)
// __attribute__((unused)); static int _map_result_field_to_name(PyObject
// *result, const char *target, const ValueName mapping[], int n, const char
// *not_found) {
//     int i = _find_result_index(result, target);
//     if (i >= 0) {
//         PyObject *t = PySequence_GetItem(result, i);
...
//     }
// }
static int _map_result_field_to_name(json &result, const char *target, const ValueName mapping[], int n, const char *not_found) {
    int val = result[target];
    const char *name = search_name(mapping, n, val);
    if (name == NULL) // not found
        name = not_found;
    result[target] = name;
...
static unsigned int _decode_by_bit(int start, int bitlength, const char *b) __attribute__((unused));
static unsigned int _decode_by_bit(int start, int bitlength, const char *b) {
    unsigned int rt = 0;
    int temp = 0;
    assert(bitlength <= 32);
...
static int _decode_by_fmt(const Fmt fmt[], int n_fmt, const char *b, int offset, int length, json &result) __attribute__((unused));
static int _decode_by_fmt(const Fmt fmt[], int n_fmt, const char *b, int offset, int length, json &result) {
    int n_consumed = 0;
    ...
    for (int i = 0; i < n_fmt; i++) {
        ...
        case UINT: {
            ...
            // object
            if (fmt[i].len <= 4)
                result[fmt[i].field_name] = ii;
            else
                result[fmt[i].field_name] = iiii;
        ...
        case BYTE_STREAM: {
            ...
            result[fmt[i].field_name] = ascii_data;
            n_consumed += fmt[i].len;
            break;
        }
        ...
        case PLMN_MK1: {
            ...
            const char *plmn = p;
            char buf[100];
            sprintf(buf, "%d%d%d-%d%d%d", plmn[0], plmn[1], plmn[2], plmn[3], plmn[4], plmn[5]);
            result[fmt[i].field_name] = std::string(buf);
            n_consumed += fmt[i].len;
            break;
        }
        ...
        case PLMN_MK2: {
            ...
            const char *plmn = p;
            char buf[100];
            sprintf(buf, "%d%d%d-%d%d", plmn[0] & 0x0F, (plmn[0] >> 4) & 0x0F, plmn[1] & 0x0F, plmn[2] & 0x0F, (plmn[2] >> 4) & 0x0F);
            // MNC can have two or three digits
            int last_digit = (plmn[1] >> 4) & 0x0F;
            if (last_digit < 10) // last digit exists
                sprintf(buf, "%s%d", buf, last_digit);
            n_consumed += fmt[i].len;
            result[fmt[i].field_name] = std::string(buf);
            break;
        }
        ...
        case QCDM_TIMESTAMP: {
            ...
            // int useconds = (double(iiii) / PER_USECOND) - double(seconds)
            // * 1.0e6; PyObject *epoch = PyDateTime_FromDateAndTime(1980, 1, 6,
            // 0, 0, 0, 0); PyObject *delta = PyDelta_FromDSU(0, seconds,
            // useconds); decoded = PyNumber_Add(epoch, delta);
            const unsigned long long PER_SECOND = 52428800;
            const double PER_USECOND = 52428800.0 / 1.0e6;
            const unsigned long long GPS_TIME = 315964800;
            assert(fmt[i].len == 8);
            unsigned long long iiii = *((unsigned long long *)p);
            unsigned long long seconds = iiii / PER_SECOND;
            unsigned long long useconds = (iiii - seconds * PER_SECOND) / PER_USECOND;
            result[fmt[i].field_name] = (seconds + GPS_TIME) * 1000000 + useconds;
            n_consumed += fmt[i].len;
            break;
        }
        ...
        case RSRP: {
            ...
            assert(fmt[i].len == 2);
            short val = *((short *)p);
            result[fmt[i].field_name] = val * 0.0625 - 180;
            n_consumed += fmt[i].len;
            break;
        }
        ...
        case RSRQ: {
            ...
            assert(fmt[i].len == 2);
            short val = *((short *)p);
            result[fmt[i].field_name] = val * 0.0625 - 30;
            n_consumed += fmt[i].len;
            break;
        }
        ...
        case WCDMA_MEAS: { // (x-256) dBm
            assert(fmt[i].len == 1);
            unsigned int ii = *((unsigned char *)p);
            result[fmt[i].field_name] = (int)ii - 256;
            n_consumed += fmt[i].len;
            break;
        }
