...
static int _decode_lte_phy_bplmn_cell_request_payload(const char *b, int offset,
                                                      size_t length,
                                                      json &result) {
    int start = offset;
    ...
    case 4: {
        offset +=
            _decode_by_fmt(LtePhyBplmnCellRequest_Payload_v4,
        ...
        _replace_result_int(result, "Barred Status", iBarredStatus);
        (void)_map_result_field_to_name(
            result, "Barred Status", LtePhyBplmnCellRequest_BarredStatus,
        ...
        _replace_result_int(result, "Q Rx Lev Min Offset", iQRxLevMinOffset);
        _replace_result_int(result, "P Max", iPMax);

        if (iStandrdsVersion == 1) {
            offset +=
                _decode_by_fmt(LtePhyBplmnCellRequest_Rel9Info,
            ...
            _replace_result_int(result, "Rel 9 Info Q Qual Min Data",
                                iR9QMinData);
            _replace_result_int(result, "Rel 9 Info Q Qual Min Offset",
                                iR9QMinOffset);
        }
