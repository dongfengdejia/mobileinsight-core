...
static int _decode_lte_phy_cdrx_events_info_payload(const char *b, int offset,
                                                    size_t length,
                                                    json &result) {
    int start = offset;
    ...
    case 1: {
        offset +=
            _decode_by_fmt(LtePhyCdrxEventsInfo_Payload_v1,
                           ARRAY_SIZE(LtePhyCdrxEventsInfo_Payload_v1, Fmt), b,
        ...
        for (int i = 0; i < num_record; i++) {
            ...
            offset +=
                _decode_by_fmt(LtePhyCdrxEventsInfo_Record_v1,
            ...
            _replace_result_int(result_record_item, "CDRX Event", iCdrxEvent);
            (void)_map_result_field_to_name(
                result_record_item, "CDRX Event", ValueNameCDRXEvent,
                ARRAY_SIZE(ValueNameCDRXEvent, ValueName), "(MI)Unknown");
    ...
    case 2: {
        offset +=
            _decode_by_fmt(LtePhyCdrxEventsInfo_Payload_v2,
                           ARRAY_SIZE(LtePhyCdrxEventsInfo_Payload_v2, Fmt), b,
                           offset, length, result);
        int num_record = _search_result_int(result, "Num Records");
        ...
        for (int i = 0; i < num_record; i++) {
            json result_record_item;
            offset +=
                _decode_by_fmt(LtePhyCdrxEventsInfo_Record_v2,
            ...
            (void)_map_result_field_to_name(
                ...
                ARRAY_SIZE(ValueNameCDRXEvent, ValueName), "(MI)Unknown");
            utemp =
                _search_result_uint(result_record_item, "Internal Field Mask");
            ...
            std::string pystr = strInternalFieldMask.c_str();
            _replace_result(result_record_item, "Internal Field Mask", pystr);
