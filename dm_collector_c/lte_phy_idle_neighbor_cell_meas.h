...
const Fmt LtePhyIncm_Subpacket_27v2_cell[] = {
    ...
    {UINT, "Inst Measured RSRQ",
     4},             // skip 10 bits, then 10 bits, (0.0625 * x - 30) dBm
    {SKIP, NULL, 4}, // RSRI[0]
    {UINT, "Inst Measured RSSI",
     4}, // skip 11 bits, then 11 bits, (0.0625 * x - 110) dBm
    {SKIP, NULL, 4},
    {SKIP, NULL, 4},
    {SKIP, NULL, 4},
    {SKIP, NULL, 4},
...
static int _decode_lte_phy_idle_neighbor_cell_meas_payload(const char *b,
    ...
    case 1: {
        ...
        for (int i = 0; i < n_subpkts; i++) {
            ...
            offset +=
                _decode_by_fmt(LtePhyIncm_Subpacket_Header_v1,
            ...
            if (subpkt_id == 26 && subpkt_ver == 1) {
                ...
                offset += _decode_by_fmt(
                    ...
                    ARRAY_SIZE(LtePhyIncm_Subpacket_Payload_26v1, Fmt), b,
                ...
                _replace_result_int(result_subpkt, "Num Cells", num_cells);
                _replace_result_int(result_subpkt, "Num Rx Ant", num_RxAnt);
                _replace_result_int(result_subpkt, "Deplexing Mode",
                                    duplexingMode);
                (void)_map_result_field_to_name(
                    result_subpkt, "Deplexing Mode", ValueNameDuplexingMode,
                ...
                for (int j = 0; j < num_cells; j++) {
                    ...
                    offset += _decode_by_fmt(
                        LtePhyIncm_Subpacket_26v1_cell,
                        ARRAY_SIZE(LtePhyIncm_Subpacket_26v1_cell, Fmt), b,
                        offset, length, result_cell_item);
                    ...
                    _replace_result_int(result_cell_item, "Cell ID", iCellId);
                    _replace_result_int(result_cell_item, "CP Type", iCPType);
                    (void)_map_result_field_to_name(
                        result_cell_item, "CP Type", ValueNameCPType,
                        ARRAY_SIZE(ValueNameCPType, ValueName), "(MI)Unknown");
                    _replace_result_int(result_cell_item, "Enabled Tx Antennas",
                                        iEnabledTxAntennas);
                    _replace_result_int(result_cell_item, "TTL Enabled",
                                        iTTLEnabled);
                    (void)_map_result_field_to_name(
                        result_cell_item, "TTL Enabled", ValueNameTrueOrFalse,
                        ARRAY_SIZE(ValueNameTrueOrFalse, ValueName),
                        "(MI)Unknown");
                    _replace_result_int(result_cell_item, "FTL Enabled",
                                        iFTLEnabled);
                    (void)_map_result_field_to_name(
                        result_cell_item, "FTL Enabled", ValueNameTrueOrFalse,
                        ARRAY_SIZE(ValueNameTrueOrFalse, ValueName),
                        "(MI)Unknown");

                    result_cell.push_back(result_cell_item);
                }
                result_subpkt["Neighbor Cells"] = result_cell;
                ...
                offset += _decode_by_fmt(
                    ...
                    ARRAY_SIZE(LtePhyIncm_Subpacket_Payload_26v2, Fmt), b,
                    offset, length, result_subpkt);
                temp = _search_result_int(result_subpkt, "Num Cells");
                int num_cells = temp & 15;           // 4 bits
                int num_RxAnt = (temp >> 4) & 3;     // 2 bits
                ...
                offset += _decode_by_fmt(
                    ...
                    ARRAY_SIZE(LtePhyIncm_Subpacket_Payload_27v2, Fmt), b,
                    offset, length, result_subpkt);
                temp = _search_result_int(result_subpkt, "Num Cells");
                ...
                for (int j = 0; j < num_cells; j++) {
                    ...
                    offset += _decode_by_fmt(
                        LtePhyIncm_Subpacket_27v2_cell,
                        ARRAY_SIZE(LtePhyIncm_Subpacket_27v2_cell, Fmt), b,
                        offset, length, result_cell_item);
                    ...
                    _replace_result_int(result_cell_item, "Physical Cell ID",
                                        iPhysicalCellId);
                    ...
                    pyfloat = RSRP;
                    _replace_result(result_cell_item, "Inst Measured RSRP",
                                    pyfloat);
                    ...
                    pyfloat = RSRQ;
                    _replace_result(result_cell_item, "Inst Measured RSRQ",
                                    pyfloat);
                    ...
                    pyfloat = RSSI;
                    _replace_result(result_cell_item, "Inst Measured RSSI",
                                    pyfloat);
                ...
                offset += _decode_by_fmt(
                    ...
                    ARRAY_SIZE(LtePhyIncm_Subpacket_Payload_27v4, Fmt), b,
                    offset, length, result_subpkt);
                temp = _search_result_int(result_subpkt, "Num Cells");
                int num_cells = temp & 63; // 6 bits
                // skip 1 bit
                ...
                _replace_result_int(result_subpkt, "Serving Cell Index",
                                    servingCellIndx);
                (void)_map_result_field_to_name(
                    result_subpkt, "Serving Cell Index", ValueNameCellIndex,
