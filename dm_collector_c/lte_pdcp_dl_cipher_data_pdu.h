    ...
    case 1: {
        ...
        for (int i = 0; i < n_subpkt; i++) {
            ...
            offset += _decode_by_fmt(
                LtePdcpDlCipherDataPdu_SubpktHeader_v1,
                ARRAY_SIZE(LtePdcpDlCipherDataPdu_SubpktHeader_v1, Fmt), b,
            ...
            if (subpkt_id == 195 && subpkt_ver == 24) {
                ...
                for (int j = 0; j < iNumPDUs; j++) {
                    ...
                    offset += _decode_by_fmt(
                        LtePdcpDlCipherDataPdu_Data_v24,
                        ARRAY_SIZE(LtePdcpDlCipherDataPdu_Data_v24, Fmt), b,
                ...
                result_subpkt["PDCPDL CIPH DATA"] = result_PDUs;
            } else if (subpkt_id == 195 && subpkt_ver == 1) {
                // PDCP PDU with Ciphering 0xC3
                offset += _decode_by_fmt(
                    LtePdcpDlCipherDataPdu_SubpktPayload_v1,
                    ARRAY_SIZE(LtePdcpDlCipherDataPdu_SubpktPayload_v1, Fmt), b,
                    offset, length, result_subpkt);
                (void)_map_result_field_to_name(
                    result_subpkt, "SRB Cipher Algorithm", ValueNameCipherAlgo,
                    ARRAY_SIZE(ValueNameCipherAlgo, ValueName), "(MI)Unknown");
                ...
                for (int j = 0; j < iNumPDUs; j++) {
                    json result_pdu_item;
                    offset += _decode_by_fmt(
                        LtePdcpDlCipherDataPdu_Data_v1,
                        ARRAY_SIZE(LtePdcpDlCipherDataPdu_Data_v1, Fmt), b,
                        offset, length, result_pdu_item);
                    int temp = _search_result_int(result_pdu_item, "Cfg Idx");
                    int iCfgIdx = temp & 63;          // 6 bits
                    int iMode = (temp >> 6) & 1;      // 1 bit
                ...
                printf("(MI)Unknown LTE PDCP DL Cipher Data PDU subpkt id and "
                       "version:"
                       " 0x%x - %d\n",
